...
class AzureStorageInterpSpec extends AnyFlatSpec with WelderTestSuite {
    ...
    "gcsToLocalFile" should "be able to download a file from gcs and write to local path" in {
      forAll { (localObjectPath: RelativePath, azurePath: AzurePath) =>
        val azureStorage = CloudStorageAlg.forAzure(StorageAlgConfig(Paths.get("/work")), new FakeAzureStorageService {
          override def downloadBlob(containerName: ContainerName, blobName: BlobName, path: Path, overwrite: Boolean)(implicit ev: Ask[IO, TraceId]): IO[Unit] = createFile(path)
        })
    ...
    "localizeCloudDirectory" should "recursively download files for a given CloudStorageDirectory" in {
      forAll { (cloudStorageDirectory: CloudStorageDirectory) =>
        ...
        val azureStorage = CloudStorageAlg.forAzure(StorageAlgConfig(Paths.get("/tmp")), new FakeAzureStorageService {
