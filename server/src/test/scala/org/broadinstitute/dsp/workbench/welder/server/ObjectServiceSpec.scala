...
import org.broadinstitute.dsde.workbench.google2.{Crc32, GcsBlobName, GetMetadataResponse, GoogleStorageService}
...
class ObjectServiceSpec extends AnyFlatSpec with WelderTestSuite {
  ...
  "localize" should "be able to localize a file" in {
    forAll {
      (bucketName: GcsBucketName, blobName: GcsBlobName, bodyString: String, localFileDestination: Path) => //Use string here just so it's easier to debug
  ...
  it should "return 404 if trying to localize a non-existent file" in {
    forAll {
      ...
      """.stripMargin
        val requestBodyJson = parser.parse(requestBody).getOrElse(throw new Exception(s"invalid request body $requestBody"))
        val request = Request[IO](method = Method.POST, uri = Uri.unsafeFromString("/"), headers = fakeTraceIdHeader).withEntity(requestBodyJson)
        ...
        } yield resp shouldBe Left(NotFoundException(fakeTraceId, s"${bucketName.value}/${blobName.value} not found"))
  ...
  "checkMetadata" should "return no storage link is found if storagelink isn't found" in {
    forAll { (localFileDestination: Path) =>
      val requestBody = s"""
             ...
             |}""".stripMargin
      val requestBodyJson = parser.parse(requestBody).getOrElse(throw new Exception(s"invalid request body $requestBody"))
      val request = Request[IO](method = Method.POST, uri = Uri.unsafeFromString("/metadata"), headers = fakeTraceIdHeader).withEntity(requestBodyJson)
  ...
  it should "return RemoteNotFound if metadata is not found in GCS" in {
    forAll { (cloudStorageDirectory: CloudStorageDirectory, localBaseDirectory: LocalBaseDirectory, localSafeDirectory: LocalSafeBaseDirectory) =>
      ...
      val requestBody = s"""
                             ...
                             |}""".stripMargin
      val requestBodyJson = parser.parse(requestBody).getOrElse(throw new Exception(s"invalid request body $requestBody"))
      val request = Request[IO](method = Method.POST, uri = Uri.unsafeFromString("/metadata")).withEntity(requestBodyJson)
      val expectedBlobPath = cloudStorageDirectory.blobPath.fold("")(s => s"/${s.asString}")
      val expectedBody =
        s"""{"syncMode":"EDIT","syncStatus":"REMOTE_NOT_FOUND","storageLink":{"localBaseDirectory":"${localBaseDirectory.path.toString}","localSafeModeBaseDirectory":"${localSafeDirectory.path.toString}","cloudStorageDirectory":"${cloudStorageDirectory.container.name}${expectedBlobPath}","pattern":"\\\\.ipynb"}}"""
      val res = for {
        resp <- objectService.service.run(request).value
  ...
  it should "return SafeMode if storagelink exists in LocalSafeBaseDirectory" in {
    forAll { (cloudStorageDirectory: CloudStorageDirectory, localBaseDirectory: LocalBaseDirectory, localSafeDirectory: LocalSafeBaseDirectory) =>
      ...
      val expectedBlobPath = cloudStorageDirectory.blobPath.fold("")(s => s"/${s.asString}")
      val expectedBody =
        s"""{"syncMode":"SAFE","storageLink":{"localBaseDirectory":"${localBaseDirectory.path.toString}","localSafeModeBaseDirectory":"${localSafeDirectory.path.toString}","cloudStorageDirectory":"${cloudStorageDirectory.container.name}${expectedBlobPath}","pattern":"\\\\.ipynb"}}"""
      val res = for {
        resp <- objectService.service.run(request).value
        body <- resp.get.body.through(text.utf8.decode).compile.foldMonoid
      } yield {
  ...
  it should "return SyncStatus.LIVE if crc32c matches" in {
    forAll { (cloudStorageDirectory: CloudStorageDirectory, localBaseDirectory: LocalBaseDirectory, localSafeDirectory: LocalSafeBaseDirectory) =>
      ...
      val expectedBlobPath = cloudStorageDirectory.blobPath.fold("")(s => s"/${s.asString}")
      val expectedBody =
        s"""{"syncMode":"EDIT","syncStatus":"LIVE","lastLockedBy":null,"storageLink":{"localBaseDirectory":"${localBaseDirectory.path.toString}","localSafeModeBaseDirectory":"${localSafeDirectory.path.toString}","cloudStorageDirectory":"${cloudStorageDirectory.container.name}${expectedBlobPath}","pattern":"\\\\.ipynb"}}"""
  ...
  it should "return SyncStatus.LocalChanged if crc32c doesn't match but local cached generation matches remote" in {
    forAll { (cloudStorageDirectory: CloudStorageDirectory, localBaseDirectory: LocalBaseDirectory, localSafeDirectory: LocalSafeBaseDirectory) =>
      ...
      val storageService = FakeGoogleStorageService(metadataResp)
      val googleStorageAlg = Ref.unsafe[IO, CloudStorageAlg](CloudStorageAlg.forGoogle(StorageAlgConfig(Paths.get("/tmp")), storageService))
      ...
      val expectedBlobPath = cloudStorageDirectory.blobPath.fold("")(s => s"/${s.asString}")
      val expectedBody =
        s"""{"syncMode":"EDIT","syncStatus":"LOCAL_CHANGED","lastLockedBy":null,"storageLink":{"localBaseDirectory":"${localBaseDirectory.path.toString}","localSafeModeBaseDirectory":"${localSafeDirectory.path.toString}","cloudStorageDirectory":"${cloudStorageDirectory.container.name}${expectedBlobPath}","pattern":"\\\\.ipynb"}}"""
      // Create the local base directory
      val directory = new File(s"/tmp/${localBaseDirectory.path.toString}")
      if (!directory.exists) {
        directory.mkdirs
  ...
  it should "return SyncStatus.RemoteChanged if crc32c doesn't match and local cached generation doesn't match remote" in {
    forAll { (cloudStorageDirectory: CloudStorageDirectory, localBaseDirectory: LocalBaseDirectory, localSafeDirectory: LocalSafeBaseDirectory) =>
      ...
      val expectedBlobPath = cloudStorageDirectory.blobPath.fold("")(s => s"/${s.asString}")
      val expectedBody =
        s"""{"syncMode":"EDIT","syncStatus":"REMOTE_CHANGED","lastLockedBy":null,"storageLink":{"localBaseDirectory":"${localBaseDirectory.path.toString}","localSafeModeBaseDirectory":"${localSafeDirectory.path.toString}","cloudStorageDirectory":"${cloudStorageDirectory.container.name}${expectedBlobPath}","pattern":"\\\\.ipynb"}}"""
      // Create the local base directory
      val directory = new File(s"/tmp/${localBaseDirectory.path.toString}")
      if (!directory.exists) {
        directory.mkdirs
  ...
  it should "return SyncStatus.OutOfSync if crc32c doesn't match and no local cached metadata found for the file" in {
    forAll { (cloudStorageDirectory: CloudStorageDirectory, localBaseDirectory: LocalBaseDirectory, localSafeDirectory: LocalSafeBaseDirectory) =>
      ...
      val expectedBlobPath = cloudStorageDirectory.blobPath.fold("")(s => s"/${s.asString}")
      val expectedBody =
        s"""{"syncMode":"EDIT","syncStatus":"DESYNCHRONIZED","lastLockedBy":null,"storageLink":{"localBaseDirectory":"${localBaseDirectory.path.toString}","localSafeModeBaseDirectory":"${localSafeDirectory.path.toString}","cloudStorageDirectory":"${cloudStorageDirectory.container.name}${expectedBlobPath}","pattern":"\\\\.ipynb$$"}}"""
      // Create the local base directory
      val directory = new File(s"/tmp/${localBaseDirectory.path.toString}")
      if (!directory.exists) {
        directory.mkdirs
  ...
  it should "return lastLockedBy if metadata shows it's locked by someone" in {
    forAll {
      (
        ...
        val localPath = s"${localBaseDirectory.path.toString}/test.ipynb"
        val bodyBytes = "this is great!".getBytes("UTF-8")
        val metadataResp = GetMetadataResponse.Metadata(
          Crc32("aZKdIw=="),
          Map("lastLockedBy" -> lockedBy.value, "lockExpiresAt" -> Long.MaxValue.toString),
          0L
        ) //This crc32c is from gsutil
        val storageService = FakeGoogleStorageService(metadataResp)
        ...
        val expectedBlobPath = cloudStorageDirectory.blobPath.fold("")(s => s"/${s.asString}")
        val expectedBody =
          s"""{"syncMode":"EDIT","syncStatus":"LIVE","lastLockedBy":"${lockedBy.value}","storageLink":{"localBaseDirectory":"${localBaseDirectory.path.toString}","localSafeModeBaseDirectory":"${localSafeDirectory.path.toString}","cloudStorageDirectory":"${cloudStorageDirectory.container.name}${expectedBlobPath}","pattern":"\\\\.ipynb"}}"""
        // Create the local base directory
        val directory = new File(s"/tmp/${localBaseDirectory.path.toString}")
        if (!directory.exists) {
          directory.mkdirs
  ...
  it should "return not return lastLockedBy if lock has expired" in {
    forAll {
      (
        ...
        val metadataResp = GetMetadataResponse.Metadata(
          Crc32("aZKdIw=="),
          Map("lastLockedBy" -> lockedBy.value, "lockExpiresAt" -> Long.MinValue.toString),
          0L
        ) //This crc32c is from gsutil
        val storageService = FakeGoogleStorageService(metadataResp)
        val objectService = initObjectService(Map(localBaseDirectory.path -> storageLink), Map.empty, Some(storageService))
        ...
        val storageAlg = new MockCloudStorageAlg {
          ...
          override def delocalize(
            ...
            IO(localObjectPath.asPath.toString shouldBe localPath) >>
              IO(gsPath.container.name shouldBe (cloudStorageDirectory.container.name)) >>
              IO(gsPath.blobPath shouldBe (getFullBlobName(localBaseDirectory.path, Paths.get(localPath), cloudStorageDirectory.blobPath))) >>
      ...
      (
        ...
        val storageAlg = new MockCloudStorageAlg {
          override def delocalize(
              ...
              generation: Long,
              userDefinedMeta: Map[String, String]
          )(implicit ev: Ask[IO, TraceId]): IO[Option[DelocalizeResponse]] = IO.raiseError(fail("delocalize shouldn't happen"))
        ...
        val res = for {
          _ <- Stream
            ...
            .drain //write to local file
          resp <- objectService.service.run(request).value
        } yield resp.get.status shouldBe Status.NoContent
        res.unsafeRunSync()
    }
  }
      ...
      (
        ...
        val storageAlg = new MockCloudStorageAlg {
          ...
          override def delocalize(
            ...
            ) >> // even thought generation in local cache is not 111L, we delocalize the file with generation being 0L to recreate the file
              IO(Some(DelocalizeResponse(112L, Crc32("newHash"))))
          override def localizeCloudDirectory(
              localBaseDirectory: RelativePath,
              cloudStorageDirectory: CloudStorageDirectory,
        ...
        val res = for {
          ...
          remoteFile <- FakeGoogleStorageInterpreter
            .getBlobBody(
              ...
              getFullBlobName(localSafeDirectory.path, Paths.get(localPath), cloudStorageDirectory.blobPath).asGcs
            )
            .compile
            .toList
        } yield {
  ...
  it should "throw GenerationMismatch exception if remote file has changed" in {
    forAll {
      (
        ...
        val bodyBytes = "this is great!".getBytes("UTF-8")
        val googleStorageAlg = CloudStorageAlg.forGoogle(StorageAlgConfig(Paths.get("/tmp")), GoogleStorageServiceWithFailures)
        val storageLinkAlg = StorageLinksAlg.fromCache(storageLinksCache)
        val metadataCacheAlg = new MetadataCacheInterp(metaCache)
        val permitsRef = Ref.unsafe[IO, Map[RelativePath, Semaphore[IO]]](Map.empty[RelativePath, Semaphore[IO]])
  ...
  "/delete" should "return StorageLinkNotFoundException" in {
    forAll { (localFileDestination: Path) =>
      ...
      val res = for {
        resp <- objectService.service.run(request).value
      } yield resp.get.status shouldBe Status.Ok
      res.attempt.unsafeRunSync() shouldBe (Left(StorageLinkNotFoundException(fakeTraceId, s"No storage link found for ${localFileDestination.toString}")))
    }
  }
  ...
  it should "return Delete a file from GCS" in {
    forAll {
      (
        ...
        val fullBlobName = getFullBlobName(localBaseDirectory.path, Paths.get(localPath), cloudStorageDirectory.blobPath).asGcs
        val res = for {
          _ <- Stream
            .emits(bodyBytes)
            .covary[IO]
            ...
            .compile
            .drain //write to local file
          _ <- FakeGoogleStorageInterpreter.createBlob(cloudStorageDirectory.container.asGcsBucket, fullBlobName, bodyBytes, "text/plain").compile.drain
          fileExisted <- FakeGoogleStorageInterpreter.getBlobBody(cloudStorageDirectory.container.asGcsBucket, fullBlobName).compile.toList
          _ <- objectService.service.run(request).value.attempt
          _ <- IO((new File(localPath)).delete())
          remoteFile <- FakeGoogleStorageInterpreter.getBlobBody(cloudStorageDirectory.container.asGcsBucket, fullBlobName).compile.toList
  ...
  "acquireLock" should "should not be able to acquire lock object doesn't exist in GCS" in {
    forAll { (storageLink: StorageLink) =>
      val objectService = initObjectService(
        Map(storageLink.localBaseDirectory.path -> storageLink),
        Map.empty,
        None
      ...
      val requestBody = s"""
                             ...
                             |}""".stripMargin
      val requestBodyJson = parser.parse(requestBody).getOrElse(throw new Exception(s"invalid request body $requestBody"))
      val request = Request[IO](method = Method.POST, uri = Uri.unsafeFromString("/lock"), headers = fakeTraceIdHeader).withEntity(requestBodyJson)
      ...
      val res = for {
        ...
        val gsPath = CloudBlobPath(storageLink.cloudStorageDirectory.container, fullBlobPath)
    ...
    forAll { (storageLink: StorageLink) =>
      ...
      val res = for {
        _ <- FakeGoogleStorageInterpreter
          .createBlob(storageLink.cloudStorageDirectory.container.asGcsBucket, fullBlobPath.asGcs, bodyBytes, "text/plain", Map.empty, None, None)
          .compile
          .drain
  ...
  it should "should be able to acquire lock when no one holds the lock" in {
    forAll { (storageLink: StorageLink) =>
      ...
      val requestBody = s"""
                             ...
                             |}""".stripMargin
      val requestBodyJson = parser.parse(requestBody).getOrElse(throw new Exception(s"invalid request body $requestBody"))
      val request = Request[IO](method = Method.POST, uri = Uri.unsafeFromString("/lock")).withEntity(requestBodyJson)
      ...
      val res = for {
        _ <- FakeGoogleStorageInterpreter
          .createBlob(storageLink.cloudStorageDirectory.container.asGcsBucket, fullBlobPath, bodyBytes, "text/plain", Map.empty, None, None)
          .compile
          .drain
        res <- objectService.service.run(request).value
        _ <- FakeGoogleStorageInterpreter
          .getBlob(storageLink.cloudStorageDirectory.container.asGcsBucket, fullBlobPath, None)
          .compile
          .lastOrError //Make sure the blob exists
      } yield res.get.status shouldBe (Status.NoContent)
      res.unsafeRunSync()
    }
  }
    ...
    forAll { (storageLink: StorageLink) =>
      ...
      val googleStorageAlg = new MockCloudStorageAlg {
        ...
        override def retrieveAdaptedGcsMetadata(localPath: RelativePath, gsPath: CloudBlobPath)(implicit ev: Ask[IO, TraceId]): IO[Option[AdaptedGcsMetadata]] =
          for {
            now <- IO.realTimeInstant
            hashedLockedBy <- IO.fromEither(hashString(lockedByString(gsPath.container, objectServiceConfig.ownerEmail)))
  ...
  it should "not be able to acquire lock when lock is owned by some other user and it hasn't expired" in {
    forAll { (storageLink: StorageLink) =>
      ...
      val googleStorageAlg = new MockCloudStorageAlg {
        ...
        override def retrieveAdaptedGcsMetadata(localPath: RelativePath, gsPath: CloudBlobPath)(implicit ev: Ask[IO, TraceId]): IO[Option[AdaptedGcsMetadata]] =
          for {
            hashedLockedBy <- IO.fromEither(hashString(lockedByString(gsPath.container, WorkbenchEmail("someoneElse@gmail.com"))))
            now <- IO.realTimeInstant
          } yield Some(AdaptedGcsMetadata(Some(Lock(hashedLockedBy, Instant.ofEpochMilli(now.toEpochMilli + 5.minutes.toMillis))), Crc32("fakecrc32"), 0L))
        override def localizeCloudDirectory(
            localBaseDirectory: RelativePath,
      ...
      } yield res shouldBe (Left(LockedByOther(fakeTraceId, s"lock is already acquired by someone else")))
      res.unsafeRunSync()
    }
  }
  ...
  it should "should be able to acquire lock when lock is owned by another user and it can't update metadata directly" in {
    forAll { (storageLink: StorageLink) =>
      ...
      val res = for {
        res <- objectService.service.run(request).value
        cache <- metaCache.get
        expectedHashedLockedBy <- IO.fromEither(
          hashString(lockedByString(storageLink.cloudStorageDirectory.container, objectServiceConfig.ownerEmail))
        )
      } yield {
  ...
  private def initObjectServiceWithPermits(permits: Map[RelativePath, Semaphore[IO]]): ObjectService = {
    ...
    val metaCache = Ref.unsafe[IO, Map[RelativePath, AdaptedGcsMetadataCache]](Map.empty)
    val defaultGoogleStorageAlg = CloudStorageAlg.forGoogle(StorageAlgConfig(Paths.get("/tmp")), FakeGoogleStorageInterpreter)
    val storageLinkAlg = StorageLinksAlg.fromCache(storageLinksCache)
    val metadataCacheAlg = new MetadataCacheInterp(metaCache)
    ObjectService(permitsRef, objectServiceConfig, Ref.unsafe(defaultGoogleStorageAlg), storageLinkAlg, metadataCacheAlg)
  }
...
class MockCloudStorageAlg extends CloudStorageAlg {
  ...
  override def retrieveUserDefinedMetadata(gsPath: CloudBlobPath)(implicit ev: Ask[IO, TraceId]): IO[Map[String, String]] = IO(Map.empty)

  override def localizeCloudDirectory(localBaseDirectory: RelativePath, cloudStorageDirectory: CloudStorageDirectory, workingDir: Path, pattern: Regex)(implicit
      ev: Ask[IO, TraceId]
  ...
  /** Delocalize user's files to GCS.
    */
  override def delocalize(localObjectPath: RelativePath, gsPath: CloudBlobPath, generation: Long, userDefinedMeta: Map[String, String])(implicit
      ev: Ask[IO, TraceId]
  ): IO[Option[DelocalizeResponse]] = ???
