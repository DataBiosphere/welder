...
class ObjectService(
  ...
  private def updateGcsMetadataAndCache(storageAlg: CloudStorageAlg, localPath: RelativePath, gsPath: GsPath, lock: Lock)(implicit
      ev: Ask[IO, TraceId]
  ): IO[Unit] =
    storageAlg
  ...
  private def delocalizeAndUpdateCache(localObjectPath: RelativePath, gsPath: GsPath, generation: Long, lock: Option[Lock])(implicit
      ev: Ask[IO, TraceId]
  ): IO[Unit] = {
